...
// Import from the CJS bundle – avoids the broken ESM barrel that tries to
// re-export non-existent style files (ERR_MODULE_NOT_FOUND on 'coy' etc.).
...
export default function Home() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('hexai_session');
    const storedConvs = localStorage.getItem('hexai_conversations');
  ...
  const handleSend = useCallback(async () => {
    ...
    if (messages.length === 0) {
      setCurrentTitle(currentInput.slice(0, 40) + (currentInput.length > 40 ? '…' : ''));
    }
    ...
    try {
      ...
      await api.streamChat(sessionId, currentInput, token => {
      ...
      setMessages(prev => [
        ...prev,
        { id: Date.now().toString(), role: 'assistant', content: '❌ Błąd połączenia z serwerem.', timestamp: new Date() },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const handleNewSession = () => {
    if (sessionId && messages.length > 0) {
      ...
      localStorage.setItem('hexai_conversations', JSON.stringify(updated));
    }
    setSessionId(null);
    setMessages([]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('audio/')) {
      ...
      try {
        ...
        setMessages(prev => [
          ...prev,
          { id: Date.now().toString(), role: 'assistant', content: `🖼️ ${description}`, timestamp: new Date() },
        ]);
      } catch {}
    }
  };
  ...
  const vramPct = stats?.vram_total_gb && stats?.vram_used_gb
    ? Math.round((stats.vram_used_gb / stats.vram_total_gb) * 100)
    : null;
  ...
  return (
    <>
      <Head>
        ...
        {/* Google Fonts for browser dev; Tauri uses system fallbacks defined in CSS */}
      ...
      <div className="app-shell">
        ...
        <aside className={`sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
          <div className="sidebar-header">
            ...
            <button className="icon-btn" onClick={() => setSidebarOpen(false)} title="Ukryj panel">
          ...
          <div className="conversations-list">
            {conversations.length === 0 && (
              <div className="conv-empty">Brak poprzednich rozmów</div>
            )}
            {conversations.map(c => (
            ...
            {stats && (
              <div className="stats-panel">
                <div className="stat-row">
                  <span>Model</span>
                  <span className={`stat-badge ${stats.model_loaded ? 'loaded' : 'idle'}`}>
                    {stats.model_loaded ? 'aktywny' : 'nieaktywny'}
                  </span>
                </div>
                <div className="stat-row">
                  <span>Sesje</span>
                ...
                {vramPct !== null && (
                  <div className="stat-vram">
                    <div className="stat-vram-label">
                      <span>VRAM</span>
                      <span>{stats.vram_used_gb?.toFixed(1)} / {stats.vram_total_gb?.toFixed(1)} GB</span>
                    </div>
                    <div className="vram-bar">
                      <div className="vram-fill" style={{ width: `${vramPct}%`, background: vramPct > 80 ? '#ef4444' : vramPct > 60 ? '#f59e0b' : '#d97706' }} />
                    </div>
                  </div>
                )}
              </div>
        ...
        {!sidebarOpen && (
          <button className="sidebar-toggle" onClick={() => setSidebarOpen(true)} title="Pokaż panel">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {messages.length === 0 && (
              <div className="empty-state">
                ...
                <div className="suggestions">
                  {['Napisz funkcję w Pythonie', 'Wyjaśnij kwantowe splątanie', 'Pomóż z debugowaniem'].map(s => (
          ...
          <div className="input-area">
            <div className="input-container">
              ...
              <div className="input-actions">
                <label className="attach-btn" title="Dołącz plik">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <style jsx global>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .logo-hex { font-size: 22px; color: var(--accent); line-height: 1; }
        .logo-text { font-family: var(--font-serif); font-size: 20px; color: var(--text-primary); letter-spacing: -0.02em; }
        ...
        .new-chat-btn:hover { background: rgba(217,119,6,0.25); border-color: var(--accent); }
        ...
        .conv-empty { color: var(--text-muted); font-size: 12px; padding: 8px 8px; }
        .conv-item {
        ...
        .stats-panel {
          ...
          border-radius: var(--radius-xs); padding: 8px 10px; margin: 8px 8px 4px;
        }
        .stat-row {
        ...
        .stat-badge.loaded { background: rgba(34,197,94,0.15); color: #4ade80; }
        .stat-badge.idle { background: rgba(107,96,87,0.3); color: var(--text-muted); }
        ...
        .vram-fill { height: 100%; border-radius: 99px; transition: width 0.5s ease; }

        /* ── Sidebar toggle ── */
        .sidebar-toggle {
          position: fixed; left: 12px; top: 14px; z-index: 100;
          background: var(--bg-elevated); border: 1px solid var(--border);
        ...
        .chat-title-text { font-family: var(--font-serif); font-size: 17px; color: var(--text-primary); }
        .mode-chip {
          font-size: 10px; font-weight: 600; letter-spacing: 0.05em;
          background: var(--accent-glow); border: 1px solid var(--accent-dim);
          color: var(--text-accent); padding: 1px 7px; border-radius: 99px;
        }
        ...
        .empty-hex { font-size: 48px; color: var(--accent); margin-bottom: 16px; line-height: 1; }
        .empty-state h2 { font-family: var(--font-serif); font-size: 28px; color: var(--text-primary); margin-bottom: 8px; }
        ...
        .suggestion-chip:hover { background: var(--bg-hover); color: var(--text-primary); border-color: var(--border); }
        ...
        .md-li { margin: 3px 0; }
        .md-h1 { font-family: var(--font-serif); font-size: 22px; margin: 12px 0 6px; color: var(--text-primary); }
        .md-h2 { font-family: var(--font-serif); font-size: 18px; margin: 10px 0 5px; color: var(--text-primary); }
        ...
        .inline-code {
          ...
          background: var(--bg-elevated); border: 1px solid var(--border);
          padding: 1px 5px; border-radius: 4px; color: var(--text-accent);
        }
        ...
        .code-header {
          display: flex; justify-content: space-between; align-items: center;
          background: #282c34; padding: 6px 12px;
        ...
        .copy-btn {
          ...
          cursor: pointer; font-family: var(--font-sans); padding: 2px 6px;
